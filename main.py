...
def roulette():
    ...
    if not user or not color or not bet:
        return {"error": "Использование: /roulette?user=ник&color=red|black|green&bet=число"}
    try:
        bet = int(bet)
    except ValueError:
        return {"message": "Ставка должна быть числом!"}

    color = color.lower()
    if color not in ["red", "black", "green"]:
        return {"message": "Цвет должен быть red, black или green!"}

    balance = get_balance(user)
    if balance <= 0:
        return {"message": f"{user}, у тебя 0 баланса. Жди восстановления!"}

    if bet > balance:
        return {"message": f"{user}, ставка превышает баланс ({balance})."}
...
def balance():
    user = request.args.get("user")
    if not user:
        return {"error": "Укажи ?user=твой_ник"}
    balance = get_balance(user)
    return {"message": f"{user}, твой баланс: {balance}."}

if __name__ == "__main__":
    # Render требует слушать порт 8080
